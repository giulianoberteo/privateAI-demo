...
- **Live lab alerts** — ask the chatbot about live operational data from VCF Operations (Aria Ops) in plain English (*"how's my lab?"*, *"any critical alerts?"*, *"check my lab for alerts"*). The agent detects operational intent automatically, fetches data directly from the Aria Ops REST API, and skips the documentation index entirely for those queries. Each alert is shown with a traffic-light severity icon (🔴 CRITICAL · 🟠 IMMEDIATE · 🟡 WARNING · 🟢 INFORMATION). The LLM responds as an **Operations monitoring assistant** (not a VCF Architect), grouping alerts by severity and suggesting remediation steps. Follow-up prompts (*"group by severity"*, *"create a table"*) continue in alert mode without re-triggering a documentation search. Set `VCF_OPS_URL`, `VCF_OPS_USER`, and `VCF_OPS_PASS` before starting Streamlit to enable this feature.
